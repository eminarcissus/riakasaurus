...
class Tests(unittest.TestCase):
    ...
    def test_2i(self):
        for i in range(100):
            m = yield self.map_bucket.fetch_datatype('test_map%d' %i)
            v = m.counters['test_counter']
            v.increment(6)
            yield m.update()
        keys = yield self.map_bucket.get_index('$bucket',self.map_bucket.name,max_results=10)
        next_page = keys.get('continuation','')
        while next_page:
            for key in keys['keys']:
                print keys
            keys = yield self.map_bucket.get_index('$bucket',self.map_bucket.name,max_results=10,continuation=next_page)
            next_page = keys.get('continuation','')
